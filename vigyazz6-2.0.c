...
int main(){
	...
	while(jatek_vege==0){
		...
		for(;lapdb>0;lapdb--){
			...
			/** 1. játékos lapjának lerakása */
/*			
/*			masol(tabla2, tabla); /** állás elmentése */
/*			temp = hozzarak(jatrak[sorr[0]], tabla, jat[sorr[0]]); /** lap lerakása */
/*			if(temp!=0){ /** elvitt lapok vizsgálata */
/*				jat[sorr[0]].eredm += temp;
				i = 0;
				while(i<4 && tabla[i][0]==tabla2[i][0]){
					i++; /** megf. sor keresése */
/*				}
				if(i==4){ /** nem találta a sort */
/*					printf("Hiba. Add meg az elvitt lapok számát: ");
					...
					for(i=0;i<temp;i++){
						do{
							scanf("%d", &j);
							if(j==-111){atir(tabla, &lapdb, &kezdlapdb, &jatdb, &jatek_vege, ossz_lap, &letesz, &korssz, &min);}
						} while(j==-111);
						ossz_lap[j-1] = sorr[0];
					}
				} else{ /** elvitt lapok feljegyzése */
/*					j = 0;
...
*/			for(i=/*1*/0;i<jatdb;i++){ /** játékosok lapjainak lerakása */
				masol(tabla2, tabla);
				temp = hozzarak(jatrak[sorr[i]], tabla, jat[sorr[i]]);
				if(temp!=0){ /** elvitt lapok vizsgálata */
					jat[sorr[i]].eredm += temp;
					j = 0;
					while(j<4 && tabla[j][1]==tabla2[j][1]){
						j++; /** megf. sor keresése */
					}
